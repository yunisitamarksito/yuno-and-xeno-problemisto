...
-- =====================================================================
-- MODULE LOADING WITH IDENTITY FIX
-- =====================================================================
...
local function GetPlayerCollectionPetsByUserId(userId)
    ...
    if player then
        ...
        local GetProfile = RouterClient.get("PlayerProfileAPI/FetchProfile")
...
task.spawn(function()
    pcall(function()
        ...
        local ok3, arr = pcall(function() return HttpService:JSONDecode(body) end)
...
do
    ...
    local suggestRemoveEvent = RouterClient.get_event("TradeAPI/SuggestRemoveItem")
...
local function partnerAutoAction()
    ...
    while TradeApp.lock_countdown and TradeApp.lock_countdown.is_going and TradeApp.lock_countdown:is_going() do task.wait(0.1) end
...
for i, tabName in ipairs(tabs) do
    ...
    tabFrame.BackgroundTransparency = 1; tabFrame.Visible = i==1; tabFrame.Parent = mainFrame; tabFrames[tabName] = tabFrame
    local cn = tabName; tabButton.MouseButton1Click:Connect(function() setActiveTab(cn) end)
...
-- Fake Player Controls in Control tab
do
...
do
    ...
    -- Reactions + Auto Spectators toggles
...
local function BPCreateInventoryItem(itemId, category, properties)
...
local DoNeonFusionRF = game:GetService("ReplicatedStorage"):FindFirstChild("DoNeonFusion", true) or findNeonFusionRF()
...
local function makePetActionBtn(labelText, xPos, parent)
    local btn = Instance.new("TextButton")
...
local petActionRow = Instance.new("Frame")
...
local spawnInvBtn = makePetActionBtn("Spawn in Inv", 0,    petActionRow)
local addTradeBtn = makePetActionBtn("Add to Trade", 0.52, petActionRow)
...
spawnInvBtn.MouseButton1Click:Connect(function()
    ...
    if name == "" then        if HintApp then HintApp:hint({ text = "Enter a pet name first.", length = 2, overridable = true }) end
        return
    end
...
print("✅ Xeno Executor Desktop - Full Script Loaded Successfully!")
